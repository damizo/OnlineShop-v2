...
specified in the orders the values are concatenated together during signing. To
...
In orders to generate the string that is signed with a key, the client MUST take
the values of each HTTP header specified by `headers` in the orders they appear.
