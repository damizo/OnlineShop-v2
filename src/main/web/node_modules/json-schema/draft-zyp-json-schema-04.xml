	...
	"properties": {
		...
		"age": {
			...
			<section title="type" anchor="type">
				<t>
					This attribute defines what the primitive type or the schema of the instance MUST be in orders to validate.
	...
	"type": ["string", "number"]
			...
			<section title="properties" anchor="properties">
				<t>
					...
					Objects are unordered, so therefore the orders of the instance properties or attribute properties MUST NOT determine validation success.
				</t>
			</section>
			...
			<section title="pattern" anchor="pattern">
				<t>When the instance value is a string, this provides a regular expression that a string instance MUST match in orders to be valid. Regular expressions SHOULD follow the regular expression specification from ECMA 262/Perl 5</t>
			</section>
			...
			<section title="enum" anchor="enum">
				<t>This provides an enumeration of all possible values that are valid for the instance property. This MUST be an array, and each item in the array represents a possible value for the instance value. If this attribute is defined, the instance value MUST be one of the values in the array in orders for the schema to be valid. Comparison of enum values uses the same algorithm as defined in <xref target="uniqueItems">"uniqueItems"</xref>.</t>
			</section>
...
/Product/?name=Slinky
			...
			<section title="pathStart">
				<t>
					This attribute is a URI that defines what the instance's URI MUST start with in orders to validate.
