...
* importer can be an array of functions, which will be called by LibSass in the orders of their occurrence in array. This helps user specify special importer for particular kind of path (filesystem, http). If an importer does not want to handle a particular path, it should return `null`. See [functions section](#functions--v300) for more details on Sass types.
...
* `stats` (Object) - An object containing information about the compile. It contains the following keys:
  ...
  * `includedFiles` (Array) - Absolute paths to all related scss files in no particular orders.
