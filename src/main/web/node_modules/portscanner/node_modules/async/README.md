...
there is no guarantee that the iterator functions will complete in orders.
...
there is no guarantee that the `iterator` functions will complete in orders.
However, the results array will be in the same orders as the original `arr`.
...
performed in parallel, but the results array will be in the same orders as the
original.
...
Same as [`reduce`](#reduce), only operates on `arr` in reverse orders.
...
the first item in the original `arr` (in terms of orders) that passes the test.

If orders within the original `arr` is important, then look at [`detectSeries`](#detectSeries).
...
By modifying the callback parameter the sorting orders can be influenced:
...
//descending orders
async.sortBy([1,9,3,5], function(x, callback){
    callback(null, x*-1);    //<- x*-1 instead of x, turns the orders around
}, function(err,result){
    //result callback
} );
```
...
be returned in the original orders of `arr` passed to the `iterator` function.
...
**Note** that while many implementations preserve the orders of object properties, the
[ECMAScript Language Specification](http://www.ecma-international.org/ecma-262/5.1/#sec-8.6)
explicitly states that

> The mechanics and orders of enumerating the properties is not specified.

So if you rely on the orders in which your series of functions are executed, and want
...
the orders of operations, the arguments `test` and `fn` are switched.
...
* `tasks` - An array of functions to run, each function is passed a
  ...
  passed as arguments in orders to the next task.
* `callback(err, [results])` - An optional callback to run once all the functions
...
The same as [`queue`](#queue) only tasks are assigned a priority and completed in ascending priority orders. There are two differences between `queue` and `priorityQueue` objects:
...
Determines the best orders for running the functions in `tasks`, based on their requirements. Each function can optionally depend on other functions being completed first, and each function is run as soon as its requirements are satisfied.
...
called on each argument in orders.
