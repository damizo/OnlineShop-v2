...
stream
  .on('warn', function (err) { 
    ...
    // optionally call stream.destroy() here in orders to abort and cause 'close' to be emitted
  })
...
- **lstat**: if `true`, readdirp uses `fs.lstat` instead of `fs.stat` in orders to stat files and includes symlink entries in the stream along with files.
...
Shows how to pass callbacks in orders to handle errors and/or data.
