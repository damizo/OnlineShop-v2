...
In orders for this to be of any use, we should avoid mangling standard JS
...
When you compress multiple files using this option, in orders for them to
...
- `unsafe_comps` (default: false) -- Reverse `<` and `<=` to `>` and `>=` to
  ...
  or `valueOf`. This could cause change in execution orders after operands in the
...
You can use the `--define` (`-d`) switch in orders to declare global
...
- `beautify` (default `true`) -- whether to actually beautify the output.
  ...
  when you want to generate minified code, in orders to specify additional
...
In orders to keep certain comments in the output you need to pass the
